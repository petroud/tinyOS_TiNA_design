...
module SRTreeC
{
	...
	//Used for broadcasting the aggregate selection on each round except the first one
	uses interface Packet as ReaggregatePacket;
	uses interface AMSend as ReaggregateAMSend;
	uses interface AMPacket as ReaggregateAMPacket;
	...
	// 3rd-> Used for changing epoch and completing 
	// 4th-> User for calculating and distributing aggregate functions for each node
	...
	uses interface Receive as ReaggregateReceive;
	...
	uses interface PacketQueue as ReaggregateSendQueue;
	uses interface PacketQueue as ReaggregateReceiveQueue;
}
...
implementation
{
	...
	message_t radioReaggregateSendPkt;
	
	bool RoutingSendBusy=FALSE;
	bool DistributionSendBusy=FALSE;
	bool ReaggregateSendBusy=FALSE;
	...
	//Reaggregate task
	task void sendReaggregateTask();
	task void receiveReaggregateTask();

	bool shouldReaggregate;
	...
	*/
	...
	void setReaggregateSendBusy(bool state)
	{
		atomic{
		ReaggregateSendBusy=state;
		}
	}
	...
	*/
	event void RoutingTimer.fired(){
		...
		//based to their ID multiplied by 4
	...
	*/
	...
	event void ReaggregateAMSend.sendDone(message_t * msg , error_t err)
	{
		dbg("Reaggregate", "A Reaggregation package sent... %s \n",(err==SUCCESS)?"True":"False");

		setReaggregateSendBusy(FALSE);
		
		if(!(call ReaggregateSendQueue.empty()))
		{
			post sendReaggregateTask();			
		}
	}
	...
	event message_t* ReaggregateReceive.receive( message_t * msg , void * payload, uint8_t len)
	{
		...
		msource =call ReaggregateAMPacket.source(msg);

		atomic{
		memcpy(&tmp,msg,sizeof(message_t));
		}
		enqueueDone=call ReaggregateReceiveQueue.enqueue(tmp);


		if(enqueueDone == SUCCESS)
		{
			post receiveReaggregateTask();
		}
		else
		{
			dbg("SRTreeC","ReaggregateMsg enqueue failed!!! \n");		
		}
	...
	task void sendReaggregateTask()
	{
		uint8_t mlen;
		uint16_t mdest;
		error_t sendDone, enqueueDone;
		message_t tmp;
		ReaggregateMsg *mrpkt;
		
		dbg("ReaggregateRuntime","sendReaggregateTask(): Starting....\n");


		mrpkt = (ReaggregateMsg*) (call ReaggregatePacket.getPayload(&tmp, sizeof(ReaggregateMsg)));
		if(mrpkt==NULL)
		{
			dbg("ReaggregateRuntime","sendReaggregateTask(): No valid payload... \n");
			return;
		}
		atomic{
		mrpkt->newAggregate = aggregateSelection;
		}
	
		call ReaggregateAMPacket.setDestination(&tmp, AM_BROADCAST_ADDR);
		call ReaggregatePacket.setPayloadLength(&tmp, sizeof(ReaggregateMsg));
		
		enqueueDone=call ReaggregateSendQueue.enqueue(tmp);

		if (call ReaggregateSendQueue.empty())
		{
			dbg("ReaggregateRuntime","sendReaggregateTask(): Q is empty!\n");
			return;
		}

		if(enqueueDone==SUCCESS)
		{
			if (call ReaggregateSendQueue.size()==1)
			{
				if(ReaggregateSendBusy)
				{
					dbg("ReaggregateRuntime", "sendReaggregateTask(): ReaggregateSendBusy= TRUE!!!\n");
					return;
				}
				
				radioReaggregateSendPkt = call ReaggregateSendQueue.dequeue();
			
				mlen= call ReaggregatePacket.payloadLength(&radioReaggregateSendPkt);
				mdest=call ReaggregateAMPacket.destination(&radioReaggregateSendPkt);

				if(mlen!=sizeof(ReaggregateMsg))
				{
					dbg("ReaggregateRuntime","sendReaggregateTask(): Unknown message!!!\n");
					return;
				}
				sendDone=call ReaggregateAMSend.send(mdest,&radioReaggregateSendPkt,mlen);
				
				if (sendDone== SUCCESS)
				{
					dbg("ReaggregateRuntime","sendReaggregateTask(): Send returned success!!!\n");
					setReaggregateSendBusy(TRUE);
				}
				else
				{
					dbg("ReaggregateRuntime","send failed!!!\n");
				}
			}
		...
		{
			dbg("ReaggregateRuntime","ReaggregateMsg failed to be enqueued in SendingQueue!!!\n");
		}	
